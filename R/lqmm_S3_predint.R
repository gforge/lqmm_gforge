                         ...
                         newdata)
{
  ...
  if (nq == 1) {
    ...
    ans <- data.frame(yhat = predict(object, newdata = newdata, level = level),
                      lower = LB,
                      upper = UB,
                      SE = apply(yhat, 1, sd))
  } else {
    ...
    for (j in 1:nq) {
      ...
      LB <- apply(yhat, 1, quantile, probs = alpha / 2)
      UB <- apply(yhat, 1, quantile, probs = 1 - alpha / 2)
      ans[[j]] <- data.frame(yhat = predict(tmp, newdata = newdata, level = level),
                             lower = LB,
                             upper = UB,
                             SE = apply(yhat, 1, sd))
    }
